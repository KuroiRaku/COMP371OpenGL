...
# define PI          3.14159265358979L /* pi */


//__________________________________________________________________________

/*
Modified Sphere Generation

Original Source:
https://stackoverflow.com/questions/43087620/draw-sphere-in-opengl-4-0
...
Sphere::Sphere(float x, float y, float z, float xLength, float yLength)
{
...
void Sphere::init(float xOrigin, float yOrigin, float zOrigin, float xLength, float yLength)
{
    ...
    for (ix = 0, b = -0.5 * M_PI, ib = 0; ib < nb; ib++, b += db)
        for (a = 0.0, ia = 0; ia < na; ia++, a += da, ix += 3)
        {
            ...
            y = cos(b) * sin(a);
            z = sin(b);
            sphere_pos[ix + 0] = (x+xOrigin) * xLength;
            sphere_pos[ix + 1] = (y + yOrigin)* yLength;

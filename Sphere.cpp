...
Sphere::Sphere(float x, float y, float z, float elongationAmountY)
{
...
void Sphere::init(float xOrigin, float yOrigin, float zOrigin, float elongationAmountY)
{
    GLfloat gridUnitLength = 0.4f * 2;
    // generate the sphere data
    GLfloat x, y, z, a, b, da, db = 1;
    GLfloat r = gridUnitLength * 0.25; // radius of the circle is 0.25 the length of 1 grid unit
    int ia, ib, ix, iy;
    ...
    for (ix = 0, b = -0.5 * M_PI, ib = 0; ib < nb; ib++, b += db)
        for (a = 0.0, ia = 0; ia < na; ia++, a += da, ix += 3)
        {
            ...
            y = cos(b) * sin(a);
            z = sin(b);
            sphere_pos[ix + 0] = (x + xOrigin) * r; // r = legnth in x 
            sphere_pos[ix + 1] = (y + yOrigin) * elongationAmountY; // this property controls the elongation amount
